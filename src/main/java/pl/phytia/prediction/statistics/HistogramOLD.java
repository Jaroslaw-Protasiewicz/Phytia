...
public class HistogramOLD {
	 ...
	 */
	public HistogramOLD(String name, int nbins, double min, double max) {
		m_nbins = nbins;
		m_min = min;
		m_max = max;
		m_name = name;
	...
	public void writeToFile(String fileName) throws IOException {
		...
		outfile.println("// Output from HistogramOLD class");
	...
	public void writeToExcelSheet(HSSFWorkbook wb, String histName)
			throws IOException {
		HSSFSheet sheet = wb.createSheet(histName);
		short rowIndx = 0;
		short cellIndx = 0;

		PrintWriter outfile = new PrintWriter(new FileOutputStream(histName));
		outfile.println("// Output from HistogramOLD class");
		outfile.println("// metaData: ");
		outfile.println("name \"" + m_name + "\"");
		outfile.println("bins " + m_nbins);
	...
	public void show() {
		...
		double binWidth = (m_max - m_min) / m_nbins;
		System.out.println("HistogramOLD \"" + m_name + "\", " + m_entries
				+ " entries");
